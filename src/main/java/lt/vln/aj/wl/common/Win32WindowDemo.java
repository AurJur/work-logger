package lt.vln.aj.wl.common;/* Copyright (c) 2012 Tobias Wolf, All Rights Reserved
 *
...
public class Win32WindowDemo implements WindowProc {
    ...
    protected void onMachineLocked(int sessionId) throws IOException {
        new StarterStopper().startBreak(sessionId);
...
//        LastIdleManager.setLastLockTime(LocalDateTime.now());
     ...
     * @param sessionId the session id
     */
    public void onMachineUnlocked(int sessionId) throws IOException {
        new StarterStopper().stopBreak(sessionId);
